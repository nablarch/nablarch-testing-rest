...
/**
 * セッションストアを引き継ぐためのプロセッサのファクトリクラス
 */
public class NablarchSIDManagerFactory implements RequestResponseProcessorFactory {
    private String cookieName = "NABLARCH_SID";

    @Override
    public RequestResponseProcessor create() {
        return new NablarchSIDManager(cookieName);
    }
     ...
     * リクエストのCookieとして付加する。
     */
    public static class NablarchSIDManager implements RequestResponseProcessor {
        private static final Logger LOGGER = LoggerManager.get(NablarchSIDManager.class);
        private final String cookieName;
        private String nablarchSID;

        public NablarchSIDManager(String cookieName) {
            this.cookieName = cookieName;
        }
